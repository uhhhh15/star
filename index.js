...
/* --- New: Favorites popup header styles (corresponds to <style> tag in index.js) --- */
/* 这些规则确保标题和按钮（预览、导出）在弹窗顶部良好对齐 */
/* These rules ensure good alignment of title and buttons (preview, export) at the top of the popup */
.favorites-header {
    display: flex;
...
/* Styles for the container of the return preview button and the new screenshot all preview button */
...
/* Ensure buttons in preview controls match or improve upon previous standalone return button style */
...
/* Old #favorites-return-button standalone styles can be kept as fallback, but prefer unified style within .favorites-preview-controls */
...
/* Specific styles for export dropdown menu (also defined in <style> in index.js) */
...
1.  **放置 `html2canvas.min.js`:**
    *   在你的插件目录 `public/extensions/third-party/star/` 下创建一个名为 `lib` 的文件夹。
    *   将下载的 `html2canvas.min.js` 文件放入这个 `lib` 文件夹。
    *   最终路径应该是：`public/extensions/third-party/star/lib/html2canvas.min.js`。
    *   `index.js` 中的 `H2C_LIB_PATH` 变量已设置为 `'extensions/third-party/star/lib/html2canvas.min.js'`，这是相对于 SillyTavern Web 服务器根目录的路径。**如果你的插件结构或 SillyTavern 的扩展服务方式不同，你可能需要调整此路径。**

2.  **浏览器缓存:** 修改 JS 和 CSS 后，强烈建议清理浏览器缓存或使用浏览器的无痕/隐私模式进行测试，以确保加载的是最新文件。

3.  **测试:**
    *   **单条收藏截图：** 打开收藏夹弹窗，每条收藏的右下角操作区应该会多出一个相机图标。点击它会尝试截取该收藏项并下载。
    *   **预览截图：** 点击收藏夹弹窗中的“预览”按钮进入预览模式。在聊天区域下方，除了“返回至原聊天”按钮，还会出现一个“截取所有预览消息”按钮。点击它会尝试截取整个预览聊天区域。
    *   如果 `html2canvas` 加载失败，截图按钮会显示为禁用状态，并有相应提示。

4.  **背景色问题:** `html2canvas` 在处理背景色时有时会比较棘手，尤其是当元素背景是透明或由父级继承时。代码中已尝试使用 CSS 变量获取主题背景色，并提供了备用颜色。如果截图背景不理想，可能需要调整 `captureAndDownload` 函数中 `h2cOptions.backgroundColor` 的获取方式或备用值。

5.  **文件名:** 文件名会包含聊天名称、类型（收藏项/预览）、消息ID或标识符以及时间戳。

6.  **控制台日志:** 注意观察浏览器开发者工具的控制台输出，它会提供插件加载、`html2canvas` 加载以及截图过程中的有用信息和错误提示。
